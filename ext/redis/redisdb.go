...
const (
	...
	keyPeerPhone = "gotg:peer:phone:%s"
...
// New creates a new RedisAdapter.
...
func Session(rdb *redis.Client, opts ...Option) *session.AdapterSessionConstructor {
	return session.Adapter(New(rdb, opts...))
}
...
func (r *RedisAdapter) SavePeer(p *storage.Peer) error {
	...
	for _, u := range p.Usernames {
		if u.Username != "" && u.Username != p.Username {
			pipe.Set(r.ctx, fmt.Sprintf(keyPeerUser, u.Username), fmt.Sprintf("%d", p.ID), 0)
		}
	}
	if p.PhoneNumber != "" {
		pipe.Set(r.ctx, fmt.Sprintf(keyPeerPhone, p.PhoneNumber), fmt.Sprintf("%d", p.ID), 0)
	}
...
func (r *RedisAdapter) GetPeerByPhoneNumber(phone string) (*storage.Peer, error) {
	idKey := fmt.Sprintf(keyPeerPhone, phone)
	idStr, err := r.rdb.Get(r.ctx, idKey).Result()
	if err == redis.Nil {
		return nil, nil
	}
...
func (r *RedisAdapter) DeleteStalePeers(olderThan int64) (int64, error) {
	var cursor uint64
	var count int64
	for {
		keys, next, err := r.rdb.Scan(r.ctx, cursor, "gotg:peer:*", 100).Result()
		if err != nil {
			return count, err
		}
		for _, key := range keys {
			...
			if p.LastUpdated > 0 && p.LastUpdated < olderThan {
				r.rdb.Del(r.ctx, key)
				if p.Username != "" {
					r.rdb.Del(r.ctx, fmt.Sprintf(keyPeerUser, p.Username))
				}
				for _, u := range p.Usernames {
					if u.Username != "" {
						r.rdb.Del(r.ctx, fmt.Sprintf(keyPeerUser, u.Username))
					}
				}
				if p.PhoneNumber != "" {
					r.rdb.Del(r.ctx, fmt.Sprintf(keyPeerPhone, p.PhoneNumber))
				}
				count++
			}
		}
